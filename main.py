...
from pipeline.parse_params import parse_params, LogType
from clients.run_clients import NetworkTrace, run_experiment
from analysis.ack_analysis import analyze_pcap, analyze_qlog, analyze_netlog
...
def main():
    ...
    # Run each experiment 
    (clients, experiments) = parse_params('params_local.json')
        ...
        for (trace, outputs) in output_files.items():
            ...
            for output in outputs:
                match trace.logtype:
                    ...
                    case LogType.QLOG | LogType.SQLOG:
                        analysis_res = analyze_qlog(output)
                    case LogType.NETLOG:
                        analysis_res = analyze_netlog(output)
                    case _:
                        assert(0)
                ...
                iteration += 1
                plot_segmented_trace(x, y, segments, filename=f"{trace.name}.png",
